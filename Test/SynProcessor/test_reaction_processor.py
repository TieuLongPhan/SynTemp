...
class TestReactionProcessor(unittest.TestCase):
    def test_label_reactions(self):
        reaction_dict = {"R-id": "R1", "new_reaction": "CCO.[O]>>CCO[O]"}
        ...
        self.assertTrue(
            "reactants" in processed_reaction and "products" in processed_reaction
        )
    ...
    def test_process_reaction(self):
        reaction = {"R-id": "R1", "new_reaction": "CCO>>CCO[O]", "products": "CCO[O]"}
        processed_reaction = ReactionProcessor.process_reaction(reaction)
        self.assertEqual(
            processed_reaction["total_charge_in_products"], 0
        )  # Assuming CCO[O] is neutral for this example
    ...
    def test_process_reactions_parallel(self):  # add new test case
        ...
        processed_reactions = ReactionProcessor.process_reactions_parallel(
            reaction_list, n_jobs=2
        )
    ...
    def test_sum_of_charge_in_products(self):
        ...
        processed_reactions = ReactionProcessor.sum_of_charge_in_products(
            reaction_list, n_jobs=2
        )
        self.assertTrue(
            all(
                "total_charge_in_products" in reaction
        ...
        self.assertTrue(
            all(
                reaction["total_charge_in_products"] == 0
                for reaction in processed_reactions
            )
        )
