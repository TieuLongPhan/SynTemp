...
class TestUnbalancedCharge(unittest.TestCase):
    ...
    def test_fix_unbalanced_charged_no_change(self):
        ...
        self.assertEqual(UnbalancedCharge.fix_unbalanced_charged(reaction_dict), reaction_dict)

    def test_parallel_fix_unbalanced_charge(self):
        reaction_dicts = [
            {'reactants': 'H2O', 'products': 'OH-', 'total_charge_in_products': -1, 'R-id': 'R1', 'label': 'test_label'},
            {'reactants': 'HCl', 'products': 'H+', 'total_charge_in_products': 1, 'R-id': 'R2', 'label': 'test_label'}
        ]
        fixed_reactions = UnbalancedCharge.parallel_fix_unbalanced_charge(reaction_dicts)
