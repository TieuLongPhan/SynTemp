...
class TestSMILESStandardizer(unittest.TestCase): ## add Test case
    ...
    def test_standardize_mol(self):
        ...
        standardized_mol = self.standardizer.standardize_mol(mol, remove_salts=True, uncharge=True)
    ...
    def test_standardize_smiles(self):
        standardized_smiles, _ = self.standardizer.standardize_smiles(self.test_smiles, remove_salts=True, uncharge=True)
        self.assertNotIn('[Cl-]', standardized_smiles)
        self.assertNotIn('[NH+]', standardized_smiles)

    def test_standardize_dict_smiles(self):
        data = [{'smiles': self.test_smiles, 'id': 1}]
        standardized_data = self.standardizer.standardize_dict_smiles(data, keys=['smiles'], remove_salts=True, uncharge=True, parallel=False)
    ...
    def test_parallel_standardize_dict_smiles(self):
        data = pd.DataFrame({'smiles': [self.test_smiles for _ in range(10)], 'id': range(10)})
        standardized_data = self.standardizer.standardize_dict_smiles(data, keys=['smiles'], remove_salts=True, uncharge=True, parallel=True, n_jobs=2)
