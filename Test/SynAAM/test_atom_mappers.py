...
from SynTemp.SynAAM.atom_mappers import map_with_rxn_mapper, map_with_graphormer, map_with_local_mapper, map_with_rdt, remove_atom_mapping, normalize_smiles
...
class TestMappingFunctions(unittest.TestCase):
    ...
    def test_remove_atom_mapping(self):
        ...
        unmapped_smiles = remove_atom_mapping("[O:1]=[C:2]=[N:3][CH2:4][c:5]1[cH:6][cH:7][cH:8][o:9]1>>[O:1]=[C:2]([NH:3][CH2:4][c:5]1[cH:6][cH:7][cH:8][o:9]1)[N:10]1[CH2:11][CH2:12][CH:13]([CH2:14][CH2:15][O:16][c:17]2[cH:18][cH:19][c:20]3[cH:21][c:22]2[CH2:23][CH2:24][c:25]2[cH:26][n:27][cH:28][c:29]([cH:30]2)[NH:31][c:32]2[n:33][cH:34][c:35]([Cl:36])[c:37]([n:38]2)[NH:39]3)[CH2:40][CH2:41]1")
        self.assertEqual(normalize_smiles(unmapped_smiles), normalize_smiles(self.smiles))
    ...
    def test_normalize_smiles(self):
        ...
        self.assertIsInstance(Chem.rdChemReactions.ReactionFromSmarts(normalized_smiles), Chem.rdChemReactions.ChemicalReaction)
