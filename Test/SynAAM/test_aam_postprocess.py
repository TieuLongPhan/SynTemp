...
class TestAMMPostprocessor(unittest.TestCase):
    ...
    def test_extract_mappings_and_count_atoms(self):
        mappings, count = AMMPostprocessor.extract_mappings_and_count_atoms(self.smiles_with_mapping)
        self.assertEqual(count, 41)

    def test_is_consistent_mapping(self):
        # Testing with a consistent reaction mapping
        self.assertTrue(AMMPostprocessor.is_consistent_mapping(self.reaction_smiles_with_mapping))
        
        # Testing with an inconsistent reaction mapping
        inconsistent_reaction_smiles = "[CH3:1][CH2:2][OH:3]>>[CH2:1]=[CH2:2]"
        self.assertFalse(AMMPostprocessor.is_consistent_mapping(inconsistent_reaction_smiles))
    ...
    def test_parallel_postprocess(self):
        mapped_smiles_list = [
            {"mapper1": self.reaction_smiles_with_mapping, "mapper2": self.reaction_smiles_with_mapping},
            {"mapper1": self.reaction_smiles_with_mapping, "mapper2": self.reaction_smiles_with_mapping}
        ]
        mapper_names = ["mapper1", "mapper2"]
        # Measure execution time with a single job
        start_time = time.time()
        _ = AMMPostprocessor.parallel_postprocess(mapped_smiles_list, mapper_names, 2, n_jobs=2, verbose=1)
