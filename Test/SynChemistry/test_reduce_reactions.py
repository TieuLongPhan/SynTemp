...
class TestReduceReactions(unittest.TestCase):
    ...
    def test_process_list_of_rsmi(self):
        ...
        self.assertEqual(ReduceReactions.process_list_of_rsmi(reactions), expected_results)

    def test_process_list_of_dicts(self):
        # Check processing on list of dictionaries
        database = [{'reactions': ['CC>>CC', 'CC>>CCC', 'C>>C']}, {'reactions': ['CCC>>CCC']}]
        expected = [
            {'reactions': ['CC>>CC', 'C>>C']},
            {'reactions': ['CCC>>CCC']}
        ]
        self.assertEqual(ReduceReactions.process_list_of_dicts(database, 'reactions'), expected)
