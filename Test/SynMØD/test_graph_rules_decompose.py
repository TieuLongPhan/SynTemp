...
from SynTemp.SynMØD.graph_rules_decompose import GraphRuleDecompose  # Assuming the class above is saved in grap_rule_decompose.py

class TestGrapRuleDecompose(unittest.TestCase):
    ...
    def test_get_key_by_value(self):
        ...
        self.assertEqual(GraphRuleDecompose.get_key_by_value(test_dict, 2), 'b')
        self.assertIsNone(GraphRuleDecompose.get_key_by_value(test_dict, 4))

    def test_is_connected(self):
        self.assertTrue(GraphRuleDecompose.is_connected(self.G))
        self.G.add_node(5)
        self.assertFalse(GraphRuleDecompose.is_connected(self.G))

    def test_remove_disconnected_part(self):
        self.G.add_node(5)  # Disconnected node
        connected_G = GraphRuleDecompose.remove_disconnected_part(self.G)
    ...
    def test_node_match(self):
        ...
        self.assertTrue(GraphRuleDecompose.node_match(node1_attrs, node2_attrs))
        self.assertFalse(GraphRuleDecompose.node_match(node1_attrs, node3_attrs))

    def test_edge_match(self):
        edge1_attrs = {'standard_order': 1}
        edge2_attrs = {'standard_order': -1}
        edge3_attrs = {'standard_order': 1}
        self.assertFalse(GraphRuleDecompose.edge_match(edge1_attrs, edge2_attrs))
        self.assertTrue(GraphRuleDecompose.edge_match(edge1_attrs, edge3_attrs))
    ...
    def test_find_maximum_common_subgraph(self):
        ...
        parent_graph.add_edges_from([(1, 2), (2, 3), (3, 4), (4, 1), (3,1)], standard_order=1)
        ...
        child_graph_nonsub.add_nodes_from([2, 3, 5], element='C', charge=0, aromatic=True)
        child_graph_nonsub.add_edges_from([(2, 3), (3, 5)], standard_order=1)

        # Find isomorphisms
        isomorph_sub = GraphRuleDecompose.find_maximum_common_subgraph(parent_graph, child_graph_sub,
                                                                       node_match=GraphRuleDecompose.node_match,
                                                                       edge_match=GraphRuleDecompose.edge_match)
        isomorph_nonsub = GraphRuleDecompose.find_maximum_common_subgraph(parent_graph, child_graph_nonsub,
                                                                         node_match=GraphRuleDecompose.node_match,
                                                                         edge_match=GraphRuleDecompose.edge_match)
    ...
    def test_remove_maximum_common_subgraph_edges(self):
        ...
        modified_graph = GraphRuleDecompose.remove_maximum_common_subgraph_edges(parent_graph, child_graph_sub,
                                                                                isomorph_sub, 'standard_order')

        # Assertions
        self.assertFalse(modified_graph.has_edge(1, 2))  # Edge (1, 2) should be removed
        self.assertTrue(modified_graph.has_edge(3, 4))  # Edge (3, 4) should remain as it's not part of the subgraph
        self.assertEqual(modified_graph.number_of_edges(), 2)  # Only two edges should remain
