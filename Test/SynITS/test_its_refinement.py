...
class TestUncertainRefinement(unittest.TestCase):
    def setUp(self) -> None:
        data_path = importlib.resources.files('Data').joinpath('uspto/uspto_its_incorrect.pkl.gz')

        self.data = load_from_pickle(data_path)

    def test_process_and_check_graph(self):
        _, type_of_graph = ITSRefinement.process_and_check_graph(self.data[0], 'local_mapper')
        self.assertEqual(type_of_graph, 'Single Cyclic')

        _, type_of_graph = ITSRefinement.process_and_check_graph(self.data[0], 'rxn_mapper')
        self.assertEqual(type_of_graph, 'Complex Cyclic')

        _, type_of_graph = ITSRefinement.process_and_check_graph(self.data[1], 'graphormer')
        self.assertEqual(type_of_graph, 'None')

        _, type_of_graph = ITSRefinement.process_and_check_graph(self.data[82], 'rxn_mapper') # Reaction center has no nodes
        self.assertEqual(type_of_graph, 'None')
    ...
    def test_process_dict(self):
        ...
        _, type_of_graph = ITSRefinement.process_and_check_graph(processed_data, 'ITSGraph')
        self.assertEqual(type_of_graph, 'Single Cyclic')
