...
# Configure logging
logging.basicConfig(filename='./Data/AAM/aam_processing.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
for mapper_type in mapper_types:
    for folder_name, num_columns in list_data.items():
        start_time = time.time()  # Start time for individual data processing
        save_dir = f'{root_dir}/Data/{folder_name}'
        data = load_database(f'{root_dir}/Data/AAM/{folder_name}/{folder_name}_reactions.json.gz')
        mapped_reactions = run_consensus_aam(data=data, rsmi_column='reactions', 
                                             save_dir=None, mapper_types=[mapper_type],
                                             data_name=folder_name, batch_size=50, check_balance=False, verbose=0, 
                                             rdt_jar_path=rdt_jar_path, working_dir='./')
        end_time = time.time()  # End time for individual data processing
        elapsed_time = round(end_time - start_time, 2)  # Round elapsed time to two decimal places
        time_dict[mapper_type][folder_name] = elapsed_time  # Update time dictionary
        logging.info(f"{folder_name} ({num_columns}) - {mapper_type}: {elapsed_time} seconds")
