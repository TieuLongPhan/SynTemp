...
if __name__ == "__main__":
    ...
    data = load_database("Data/Temp/data_aam.json.gz")[:10000]
    its = ITSExtraction()
    result, _ = its.parallel_process_smiles(data, mapper_names=["rxn_mapper", "local_mapper","graphormer"], n_jobs=1)
    print(result[0])
    its_h = ITSHAdjuster()
    result = its_h.process_graph_data_parallel(result, "ITSGraph", n_jobs=1, verbose=2)
    print(result[0])
