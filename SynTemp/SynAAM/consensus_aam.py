...
class ConsensusAAM:
    ...
    def __init__(self, reactions_list: List[Dict], rsmi_column: str = 'reactions', save_dir: str = './', 
    ...
    def extract_smiles(self) -> List[str]:
        ...
        return [reaction_dict[self.rsmi_column] for reaction_dict in self.reactions_list]
    def process_batch(self, batch: List[str], rxn_mapper, rdt_jar_path: str, working_dir: str) -> None:
        """
        ...
        if 'rxn_mapper' in self.mapper_types:
            results['rxn_mapper'] = [map_with_rxn_mapper(rxn, rxn_mapper) for rxn in batch]
        if 'graphormer' in self.mapper_types:
            results['graphormer'] = [map_with_graphormer(rxn) for rxn in batch]
        if 'rdt' in self.mapper_types:
            results['rdt'] = [map_with_rdt(rxn, rdt_jar_path, working_dir)  for rxn in batch]
    ...
    def fit(self, batch_size: int, rxn_mapper, rdt_jar_path: str = None, working_dir: str = None) -> List[Dict]:
        """
        ...
        for i in range(0, len(self.smiles_list), batch_size):
            ...
            logging.info(f"Processed batch {batch_number}/{len(self.smiles_list) // batch_size}")
