...
class MØDRules:
    ...
    def charge_to_string(charge):
        ...
        """
        if charge > 0:
            return '+' * charge  # Repeat the '+' symbol 'charge' times for positive charges
        elif charge < 0:
            return '-' * abs(charge)  # Repeat the '-' symbol 'abs(charge)' times for negative charges
        else:
            return ''  # No charge symbol for neutral atoms
    @staticmethod
    def convert_graph_to_gml(graph: nx.Graph, section: str, changed_node_ids: List) -> str:
        """
        ...
        if section != 'context':
            ...
            for node in graph.nodes(data=True):
                if node[0] in changed_node_ids:
                    ...
                    charge_str = MØDRules.charge_to_string(charge)
                    gml_str += f'      node [ id {node[0]} label "{element}{charge_str}" ]\n'
    ...
    @staticmethod
    def RulesGrammar(L: nx.Graph, R: nx.Graph, K: nx.Graph, rule_name: str, changed_node_ids: List) -> str:
        """
        ...
        gml_str += MØDRules.convert_graph_to_gml(L, "left", changed_node_ids)
        gml_str += MØDRules.convert_graph_to_gml(K, "context", changed_node_ids)
        gml_str += MØDRules.convert_graph_to_gml(R, "right", changed_node_ids)
        gml_str += "]"
        return gml_str

    @staticmethod
    def process_graph_rules(graph_rules: Dict[str, Tuple[nx.Graph, nx.Graph, nx.Graph]], id_column : str = 'R-id',
                            rule_column :str = 'GraphRules', reindex: bool = False, attributes: List[str] = ['charge']) -> Dict[str, str]:
        """
        ...
        changed_node_ids = MØDRules.find_changed_nodes(L, R, attributes)
        rule_grammar = MØDRules.RulesGrammar(L, R, K, rule_name, changed_node_ids)
        return rule_grammar
    
    @classmethod
    def auto_extraction(cls, data_dicts: List[Dict], id_column: str = 'R-id',
                        n_jobs: int = 4, verbose: int = 1, rule_column: str = 'GraphRules',
                        reindex: bool = False, save_path: Optional[str] = None, attributes: List[str] = ['charge']) -> List[str]:
        """
        ...
        results = Parallel(n_jobs=n_jobs, verbose=verbose)(
            delayed(cls.process_graph_rules)(data_dict, id_column=id_column,
                                             rule_column=rule_column, reindex=reindex, attributes=attributes)
    ...
    @staticmethod
    def find_changed_nodes(graph1: nx.Graph, graph2: nx.Graph, attributes: list = ['charge']) -> list:
        """
