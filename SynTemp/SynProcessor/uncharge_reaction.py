...
class UnchargeReaction:
    ...
    @staticmethod
    def random_pair_ions(charges: List[int], smiles: List[str]) -> Tuple[List[List[str]], List[List[int]]]:
        """
        ...
        for group_size in range(2, 5):  # Start with pairs, then triples, and finally quads
            while True:
                group = find_groups([i for i in indices if i not in used_indices], group_size)
                if not group:
    ...
    def uncharge_anion(smiles: str, charges: int = -1) -> str:
        ...
        # Convert the SMILES string to an RDKit molecule object
            ...
            new_smiles = smiles.replace(f'{charges}', '').replace('[', '').replace(']', '')
            return new_smiles
    ...
    def uncharge_smiles(charge_smiles: str) -> str:
        ...
        paired_smiles, paired_charges = UnchargeReaction.random_pair_ions(original_ion_charges, original_ionic_parts)
        ...
        for i_smile, i_charge in zip(paired_smiles, paired_charges):
            ...
            for perm in permutations(modified_ions):
                ...
                if Chem.MolFromSmiles(combined_ionic):
                    coordinate_pattern = ['->', '<-']
                    if all(pattern not in Chem.CanonSmiles(combined_ionic) for pattern in coordinate_pattern):
    ...
    def ammonia_hydroxide_standardize(reaction_smiles: str) -> str:
        ...
        new_smiles = reaction_smiles.replace('[NH4+].[OH-]', 'N.O').replace('[OH-].[NH4+]', 'O.N')
        return new_smiles
    
    @classmethod
    def apply_uncharge_smiles_to_reactions(cls,reactions: List[Dict[str, str]], 
                                        uncharge_smiles_func: Callable[[str], str],
                                        n_jobs: int = -1) -> List[Dict[str, str]]:
        """
        ...
        def process_reaction(reaction):
            ...
            uncharged_reactants_formula = cls.mol_to_molecular_formula(Chem.MolFromSmiles(uncharged_reactants))
            uncharged_products_formula = cls.mol_to_molecular_formula(Chem.MolFromSmiles(uncharged_products))
            ...
            reaction['standardized_reactions'] = f"{reaction['new_reactants']}>>{reaction['new_products']}"
            return reaction

        # Use joblib to parallelize the processing of reactions
        reactions = Parallel(n_jobs=n_jobs)(delayed(process_reaction)(reaction) for reaction in reactions)
        return reactions
