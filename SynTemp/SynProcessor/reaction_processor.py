...
class ReactionProcessor:
    ...
    @classmethod
    def process_reactions_parallel(cls, reaction_list: List[Dict], n_jobs: int = -1) -> List[Dict]:
        """
    ...
    def process_reaction(reaction: Dict[str, str]) -> Dict[str, Union[str, int]]:
        ...
        total_charge = sum(ReactionProcessor.calculate_charge(product) for product in products)
        reaction['total_charge_in_products'] = total_charge
        return reaction
    
    @classmethod
    def sum_of_charge_in_products(cls, reaction_list: List[Dict], n_jobs: int = -1) -> List[Dict]:
        """
