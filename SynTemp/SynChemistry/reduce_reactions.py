...
class ReduceReactions:
    ...
    def process_rsmi(reaction_smiles: str) -> Tuple[str, bool]:
        ...
        Returns:
        - Tuple[str, bool]: A tuple containing the reaction SMILES and a boolean indicating if it is balanced.
        """
    ...
    def process_list_of_rsmi(reactions: List[str]) -> List[Tuple[str, bool]]:
        """
        Processes a list of reaction SMILES strings and filters out those that are balanced.

        Parameters:
        - reactions (List[str]): A list of reaction SMILES strings.

        Returns:
        - List[Tuple[str, bool]]: A list of tuples, each containing a reaction SMILES string and a boolean for balance.
        """
    ...
    def process_list_of_dicts(cls, database: List[Dict], col_name: str) -> List[Dict]:
        """
        Processes a list of dictionaries, each containing a list of reaction SMILES strings, and updates them with balanced reactions.

        Parameters:
        - database (List[Dict]): The list of dictionaries containing reaction data.
        - col_name (str): The key in each dictionary that holds the reaction SMILES strings.
