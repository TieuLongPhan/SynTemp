...
class ITSExtraction:
    ...
    def check_equivariant_graph(its_graphs: List[nx.Graph]) -> Tuple[List[Tuple[int, int]], int]:
        ...
        for i in range(1, len(its_graphs)):
            ...
            if nx.is_isomorphic(its_graphs[0], its_graphs[i], node_match=nodeMatch, edge_match=edgeMatch):
    ...
    @staticmethod
    def process_mapped_smiles(mapped_smiles: Dict[str, str], 
                            mapper_names: List[str], 
                            check_method = 'RC', # or ITS
                            threshold: int = 2,
                            id_column: str = 'R-id',
                            ignore_aromaticity: bool = False) -> Dict[str, any]:
        """
        ...
        for mapper in mapper_names:
            ...
            rules_by_map[mapper] = GraphRuleExtraction.extract_reaction_rules(G, H, ITS, extend=False)
    ...
    def parallel_process_smiles(
        ...
        check_method = 'RC',
        export_full=False,
        ignore_aromaticity: bool = False) -> List[Dict[str, any],]:
        """
        ...
        results = Parallel(n_jobs=n_jobs, verbose=verbose)(
            delayed(ITSExtraction.process_mapped_smiles)(mapped_smiles, mapper_names, check_method, threshold, id_column, ignore_aromaticity) for mapped_smiles in mapped_smiles_list
        )
        ...
        if export_full:
            ...
            new_results_incorrect = [d for d in results_incorrect if 'ITSGraph' in d.keys()]
            
        else:
            new_results_correct = [{id_column: d[id_column], 'ITSGraph': d['ITSGraph'], 'GraphRules': d['GraphRules']} for d in results_correct if 'ITSGraph' in d.keys()]
            new_results_incorrect = [d for d in results_incorrect if 'ITSGraph' in d.keys()]
