...
class GraphRuleExtraction:
    ...
    def extract_reaction_rules(
        ...
        if extend:
            reaction_center_nodes = GraphRuleExtraction.find_nearest_neighbors(
                its_graph, reaction_center_nodes, n_knn
            )

        rules_network = tuple(
            GraphRuleExtraction.extract_subgraph(graph, list(reaction_center_nodes))
            for graph in (reactants_graph, products_graph, its_graph)
        )

        rules_graph = rules_network[2]
        rules_graph = GraphRuleExtraction.remove_normal_egdes(
