...
class GraphRuleExtraction:
    def __init__(self):
        """
        Initializes the GraphRuleExtraction class without any parameters.
        """
        pass
    ...
    @staticmethod
    def find_nearest_neighbors(G: nx.Graph, center_nodes: List[int], n_knn: int = 1) -> Set[int]:
        """
        ...
        for _ in range(n_knn):
            neighbors = set(chain.from_iterable(G.neighbors(node) for node in extended_nodes))
    ...
    @staticmethod
    def extract_reaction_rules(reactants_graph, products_graph, its_graph, extend: bool = False, n_knn: int = 1) -> Dict[str, Any]:
        """
        ...
        """
        reaction_center_nodes = GraphRuleExtraction.find_unequal_order_edges(its_graph)

        if extend:
            reaction_center_nodes = GraphRuleExtraction.find_nearest_neighbors(its_graph, reaction_center_nodes, n_knn)

        rules_network = tuple(GraphRuleExtraction.extract_subgraph(graph, list(reaction_center_nodes))
                            for graph in (reactants_graph, products_graph, its_graph))
    
        rules_graph = rules_network[2]
        rules_graph = GraphRuleExtraction.remove_normal_egdes(rules_graph, 'standard_order')
        rules_network = (rules_network[0], rules_network[1], rules_graph)
        return rules_network


    @staticmethod
    def rules_extraction(reaction_dict: Dict[str, Any], mapper_type: str = 'ITSGraph', extend: bool = True, n_knn: int = 1) -> Dict[str, Any]:
        """
    ...
    @classmethod
    def process_rules_extraction(cls, reaction_dicts: List[Dict[str, Any]], mapper_type: str = 'ITSGraph', 
                                 n_jobs: int = 4, verbose: int = 0, extend: bool = True, n_knn: int = 1) -> List[Dict[str, Any]]:
        """
        ...
        results = Parallel(n_jobs=n_jobs, verbose=verbose)(
            delayed(cls.rules_extraction)(reaction_dict, mapper_type, extend, n_knn) for reaction_dict in reaction_dicts
        )
        return results
