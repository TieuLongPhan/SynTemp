...
class ChemicalGraphVisualizer:
    def __init__(self, seed: Optional[int] = None, element_colors: Optional[Dict[str, str]] = color_scheme):
        """
    ...
    def graph_vis(self, G: nx.Graph, node_size: int = 100, visualize_edge_weight: bool = False, 
                edge_font_size: int = 10, show_node_labels: bool = False, node_label_font_size: int = 12,
        ...
        node_colors = [self.element_colors.get(G.nodes[node]['element'], '#000000') for node in G.nodes()]
        ...
        if show_node_labels:
            ...
            nx.draw(G, pos, ax=ax, with_labels=True, labels=node_labels, node_color=node_colors, node_size=node_size, 
                    font_size=node_label_font_size, font_weight='bold')
        else:
            nx.draw(G, pos, ax=ax, with_labels=False, node_color=node_colors, node_size=node_size, font_weight='bold')
    ...
    def its_vis(self, G: nx.Graph, node_size: int = 100, show_node_labels: bool = False, 
            node_label_font_size: int = 12, ax: Optional[plt.Axes] = None) -> None:
        """
        ...
        if show_node_labels:
            node_labels = {node: G.nodes[node]['element'] for node in G.nodes()}
            nx.draw(G, pos, ax=ax, with_labels=True, labels=node_labels, node_color=node_colors, node_size=node_size, 
                    font_size=node_label_font_size, font_weight='bold', edge_color=edge_colors)
        else:
            nx.draw(G, pos, ax=ax, with_labels=False, node_color=node_colors, node_size=node_size, font_weight='bold', 
                    edge_color=edge_colors)
    
    def visualize_all(self, graph_tuple, figsize=(15, 5),
                      left_graph_title="Reactants", k_graph_title="ITS Graph", right_graph_title="Products",):
        """
    ...
    def multigraph_vis(self, G: nx.MultiGraph, node_size: int = 100, 
                   show_node_labels: bool = False, node_label_font_size: int = 12, 
        ...
        nx.draw_networkx_nodes(G, pos, ax=ax, node_size=node_size, 
                            node_color=[self.element_colors.get(G.nodes[node]['element'], '#000000') for node in G.nodes()])
        ...
        for u, v, data in G.edges(data=True):
            ...
            if style == "double":
                nx.draw_networkx_edges(G, pos, ax=ax, edgelist=[(u, v)], style=":", width=2)
                nx.draw_networkx_edges(G, pos, ax=ax, edgelist=[(u, v)], style="solid", width=2, edge_color='white', alpha=1)
            elif style == "triple":
                nx.draw_networkx_edges(G, pos, ax=ax, edgelist=[(u, v)], style="solid", width=4)
            elif style == "dotted":
                nx.draw_networkx_edges(G, pos, ax=ax, edgelist=[(u, v)], style="dotted", width=2)
