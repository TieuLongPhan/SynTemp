...
class ChemicalReactionVisualizer:
    @staticmethod
    def create_html_table_with_svgs(svg_list: List[str], titles: List[str], num_cols: int = 2,
                                    orientation: str = 'vertical', title_size: int = 16) -> HTML:
        """
    ...
    @staticmethod
    def visualize_reaction(reaction_smiles: str, img_size: tuple = (600, 300), highlight_by_reactant: bool = True,
                           mol_scale: float = 0.5, bond_line_width: float = 2.0, atom_label_font_size: int = 12,
                           padding: float = 0.01, show_atom_map: bool = False) -> SVG:
        """
    ...
    @staticmethod
    def visualize_and_compare_reactions(input_dict: Dict[str, str], id_col: str = 'R-id', 
                                        img_size: tuple = (1000, 600), num_cols: int = 2, 
        ...
        for key, reaction_str in input_dict.items():
            if key != id_col:
                svg = ChemicalReactionVisualizer.visualize_reaction(reaction_str, img_size=img_size, highlight_by_reactant=True)
                svg_list.append(svg.data)
                titles.append(key)
        display(ChemicalReactionVisualizer.create_html_table_with_svgs(svg_list, titles, num_cols, orientation))