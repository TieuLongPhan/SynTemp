...
class AutoTemp:
    ...
    def extract_its(
        ...
        for i in range(num_batches):
            ...
            save_to_pickle(
                batch_correct, os.path.join(temp_dir, f"batch_correct_{i}.pkl")
            )
            save_to_pickle(
    ...
    def fit(
        ...
        if self.rebalancing:
            data = self.rebalance(
                data, self.rsmi, self.id, self.n_jobs, self.batch_size
            )
            print(data[0])
        clean_data = self.clean(data, self.id, self.rsmi, self.n_jobs)

        # Step 2: Run atom-atom mapping
        aam_data = self.run_aam(
            clean_data, self.mapper_types, "reactions", self.job_timeout, self.safe_mode
        )

        # Step 3: Extract ITS graphs and categorize them
        its_correct, its_incorrect, uncertain_hydrogen = self.extract_its(
            aam_data,
            self.mapper_types,
            self.batch_size,
            self.verbose,
        ...
        rules, reaction_dicts, templates, hier_templates = self.rule_extract(
            its_correct,
